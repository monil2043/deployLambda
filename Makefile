...
lint: format
	@echo "Running flake8"
	poetry run flake8 demo_service_rest_backend/* cdk/* tests/*
	@echo "Running mypy"
	$(MAKE) mypy-lint
...
mypy-lint:
	poetry run mypy --pretty demo_service_rest_backend cdk tests
...
build: deps
	mkdir -p .build/lambdas ; cp -r demo_service_rest_backend .build/lambdas
...
integration:
	poetry run pytest tests/integration  --cov-config=.coveragerc --cov=demo_service_rest_backend --cov-report xml

e2e:
	poetry run pytest tests/e2e  --cov-config=.coveragerc --cov=demo_service_rest_backend --cov-report xml
